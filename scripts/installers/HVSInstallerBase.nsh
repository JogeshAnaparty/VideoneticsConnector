...
InstallDir "$PROGRAMFILES\${APP_ROOT}\${APP_NAME}${PRODUCT_VERSION}"
...
!endif
  ...
  ; Put file there
  !define REGPATH "Software\${APP_ROOT}\${APP_NAME}${PRODUCT_VERSION}"
  !define REG_UN_PATH Software\Microsoft\Windows\CurrentVersion\Uninstall\${APP_NAME}${PRODUCT_VERSION}
  File /r /x *.vshost.exe.* /x *.pdb /x *.dll.config /x *.xml ${SOURCE_PATH}\*
  ...
  WriteRegDWORD HKLM ${REG_UN_PATH} "EstimatedSize" 130000
...
Section "Uninstall"
  ...
  DeleteRegKey HKLM "SOFTWARE\${APP_ROOT}\${APP_NAME}${PRODUCT_VERSION}"
